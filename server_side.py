...
def handle_client(client_socket, addr):
    ...
    while True:
        try:
            ...
            tmp_data = client_socket.recv(4096).decode('utf-8')
                ...
                if is_valid == False: #if the data is not valid, we send an error message with the incorrect protocol error code
                    ...
                    if send_response == False:
                        ...
                        try:
                            ...
                            print(f"An error occured when sending to the user {user}: {e}")
                            response = handle_errors(0x3d, "An unkown error occured", sender, SERVER_NAME)
                            sent_response = True
